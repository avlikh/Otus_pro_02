...
### 2) Выыодим версию текущего ядра
### 3) Обновляем кэш пакетов
### 4) Обновляем пакеты в системе
### 5) Устанавливаем последнюю версию ядра из стандартного репозитория Debian
### 6) Перезапускаем систему для обновления ядра
### 7) Ждем 10 секунд и проверяем доступность системы. Повторяем проверки в течении 5 минут
### 8) Показываем версию старого и нового ядра
